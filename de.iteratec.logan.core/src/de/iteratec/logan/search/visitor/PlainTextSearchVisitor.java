...
import de.iteratec.logan.search.BoyerMoore;
...
public class PlainTextSearchVisitor {
  ...
  public PlainTextSearchVisitor(TextSearchRequestor collector, String searchPattern, boolean ignoreCase) {
  ...
  public IStatus search(IFile[] files, IProgressMonitor monitor) {
    ...
    try {
      String taskName = searchPattern == null ? SearchMessages.TextSearchVisitor_filesearch_task_label : Messages
          .format(SearchMessages.TextSearchVisitor_textsearch_task_label, searchPattern);
  ...
  public boolean processFile(IFile file) {
    try {
      if (!fCollector.acceptFile(file) || searchPattern == null) {
        return true;
      }
  ...
  private void locateMatches(IFile file, CharSequence searchInput) throws CoreException {
    try {
      BoyerMoore stringSearcher = new BoyerMoore(searchPattern, ignoreCase);
      List<Integer> allMatches = stringSearcher.findAll(searchInput);

      int k = 0;
      for (Integer start : allMatches) {
